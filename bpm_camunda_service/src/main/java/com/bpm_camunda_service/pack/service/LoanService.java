...
//	@Autowired
//	private ModelMapperClass mMapper;
	@Autowired
	...
	public List<Loans> getTask(UnAssignRequest request) {
		...
		TaskCamundaResponse[] response = webClientService.postCall(GET_UNASSIGN_TASK, request,
				TaskCamundaResponse[].class);
	...
	private Loans getLoansObject(Loan loan, TaskCamundaResponse task, User emp) {
		...
		return Loans.builder().taskDetails(taskDetails).loanDetails(LoanDetails).userDetails(userDetails).build();
	}
